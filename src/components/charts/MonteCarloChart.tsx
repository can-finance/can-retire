...
export const MonteCarloChart = React.memo(function MonteCarloChart({ data, monteCarlo, inflationAdjusted }: MonteCarloChartProps) {
    const chartData = useMemo(() => {
        // Pre-compute a Map for O(1) factor lookup instead of data.find() inside the map loop
        const factorByYear = new Map<number, number>(data.map(r => [r.year, r.inflationFactor]));

        return monteCarlo.percentiles.map(d => {
            const factor = inflationAdjusted ? (factorByYear.get(d.year) ?? 1.0) : 1.0;
            return {
                year: d.year,
                age: d.age,
                p5:  d.p5  / factor,
                                ...
                                return (
                                    <div className="bg-white p-4 border border-slate-100 shadow-xl rounded-xl text-xs space-y-2">
                                        ...
                                        <div className="space-y-1">
                                            <div className="flex justify-between gap-4"><span className="text-indigo-300 font-medium">95th Percentile:</span> <span>{formatCurrencyShort(d.p95)}</span></div>
                                            <div className="flex justify-between gap-4"><span className="text-indigo-400 font-medium">75th Percentile:</span> <span>{formatCurrencyShort(d.p75)}</span></div>
                                            <div className="flex justify-between gap-4"><span className="text-indigo-600 font-bold border-t border-b border-indigo-100 py-1 my-1">Median (50th):</span> <span className="font-bold py-1 my-1 border-t border-b border-indigo-100">{formatCurrencyShort(d.p50)}</span></div>
                                            <div className="flex justify-between gap-4"><span className="text-indigo-400 font-medium">25th Percentile:</span> <span>{formatCurrencyShort(d.p25)}</span></div>
                                            <div className="flex justify-between gap-4"><span className="text-indigo-300 font-medium">5th Percentile:</span>  <span>{formatCurrencyShort(d.p5)}</span></div>
                                        </div>
                                    </div>
                                );
                            }
